...
/**
 * 生成代理类的字节码
 * 本类修改于sun.misc.ProxyGenerator，使得其可以以继承的方式生成代理类
 * 可以代理父类的私有方法，并将访问权限声明为public
 * 局限是被代理的类必须要有无参构造函数
 *
 * @modifier lenson
 * @version 1.0
...
//  private static final int opc_jsr_w                  = 201;
    ...
    /* 以下是代理中的一些额外信息，包括Proxy类和InvocationHandler */
    /** name of the superclass of proxy classes */
    private static String superclassName = "java/lang/reflect/Proxy";

    /** name of field for storing a proxy instance's invocation handler */
    private final static String handlerFieldName = "h";

    //通过环境变量决定是否将生成的class文件写到磁盘中
    ...
    /**
     * 通过调用generateProxyClass，生成代理类的字节码
     */
    public static byte[] generateProxyClass(final String name,
                                            Class<?> supClass,
                                            Class<?>[] interfaces) {
        superclassName = supClass.getName().replace(".", "/");
        return generateProxyClass(name, supClass, interfaces, (ACC_PUBLIC | /*ACC_FINAL |*/ ACC_SUPER));
    }

    /**
     * 通过调用generateClassFile，生成代理类的字节码
     * 判断环境变量，决定是否将class文件写到磁盘中
     *
     * @param name        the class name of the proxy class
     * @param interfaces  proxy interfaces
     * @param accessFlags access flags of the proxy class	代理类的访问权限
     */
    public static byte[] generateProxyClass(final String name,
                                            Class<?> supClass,
                                            Class<?>[] interfaces,
                                            int accessFlags)
    {
        ByteCodeGenerator gen = new ByteCodeGenerator(name, interfaces, accessFlags);
        final byte[] classFile = gen.generateClassFile(supClass);
    ...
    /* 先加载Object类中hashCode、equals和toString方法的Method对象，所有代理类都代理这些方法 */
    ...
    /**
     * 以方法签名为key，以存储ProxyMethod对象的list为值
     */
     ...
     */
    private ByteCodeGenerator(String className, Class<?>[] interfaces, int accessFlags) {
     ...
     * 生成代理类文件
     */
    private byte[] generateClassFile(Class<?> supClass) {

        /* ============================================================
         * Step 1: Assemble ProxyMethod objects for all methods to
         * generate proxy dispatching code for.
         * 将所有需要代理的方法组装成ProxyMethod对象，
         * 并存入到proxyMethods Map中
         */
         ...
         * proxy interfaces.
         * 所有代理类都要代理从Object继承来的hashCode、equals、toString方法
         */
         ...
         * methods.
         * 为接口生成代理方法，存入到proxyMethods Map中
         * 如果不同接口含相同方法，则前面的接口优先于后面的接口
         */
         ...
         * 添加父类的方法
         */
        for(Method m : supClass.getDeclaredMethods()) {
            if (!m.isAccessible()) {
                m.setAccessible(true);
            }
            addProxyMethod(m, supClass);
        }

        /*
         * For each set of proxy methods with the same signature,
         * verify that the methods' return types are compatible.
         * 对于每个具有相同方法签名的代理方法，校验方法的返回类型是否兼容
         * 如果签名相同的方法具有不同类型（或者非父子关系类型）的返回值，则抛出异常
         */
         ...
         * 为我们生成的类中的所有字段和方法组装FieldInfo和MethodInfo结构
         */
        try {
            //生成构造方法的MethodInfo并添加到methods中
            ...
            for (List<ProxyMethod> sigmethods : proxyMethods.values()) {
                for (ProxyMethod pm : sigmethods) {
                    ...
                    //为每个方法在代理类中添加Method类型的静态全局变量
                    fields.add(new FieldInfo(pm.methodFieldName,
                            "Ljava/lang/reflect/Method;",
                            ACC_PRIVATE | ACC_STATIC));

                    // generate code for proxy method and add it
                    //为每个ProxyMethod生成MethodInfo，并添加到methods中
                    methods.add(pm.generateMethod());
                }
            }
            //往类里添加InvocationHander字段
            ...
            /**
             * 构造静态代码块，添加到methods中
             * 静态代码块里通过反射获取所有要代理的方法，并赋值到对应的全局变量中
             */
     ...
     * method.
     *
     * 如果该方法的ProxyMethod对象不存在，就添加进proxyMethods Map里
     * 如果已经存在就直接返回之前的ProxyMethod对象
     *
        ...
        if (sigmethods != null) {
            for (ProxyMethod pm : sigmethods) {
                if (returnType == pm.returnType) {
                     ...
                     * overridden methods.
                     * 将异常类型减少到可以与两个重写方法的throws子句兼容抛出的最大异常集
                     */
                    List<Class<?>> legalExceptions = new ArrayList<>();
                    // 如果exceptionTypes是pm异常的父类，保存pm的异常
                    collectCompatibleTypes(
                            exceptionTypes, pm.exceptionTypes, legalExceptions);
                    // 如果pm异常是exceptionTypes的父类，则保存exceptionTypes
                    collectCompatibleTypes(
    ...
    private static void checkReturnTypes(List<ProxyMethod> methods) {
        ...
        /*
         * List of return typeMethodInfo generateMethod()s that are not yet known to be
         ...
         * 1、如果为原始数据类型，则抛异常；
         * 2、对于每个具有相同方法签名的方法：
         *     2.1、如果代理方法的returnType是uncoveredReturnTypes中任意类型的父类，则不添加；
         *     2.2、如果代理方法的returnType是uncoveredReturnTypes中任意类型的子类，
         *          则将returnType添加到uncoveredReturnTypes中。若已经添加，则用新类替换旧类；
         * 3、如果pm的returnType与uncoveredReturnTypes中任意类型没有继承，则将pm的returnType添加到uncoveredReturnTypes中；
         * 4、如果uncoveredReturnTypes的长度大于1，则报错，因为具有相同方法签名的方法必须要返回类型兼容，这样相当于重载。
         *    否则不知道该调用哪个方法
         */
        nextNewReturnType:
    ...
    private class ProxyMethod {
        ...
        private MethodInfo generateMethod() throws IOException {
            ...
            //将第一个引用类型本地变量压入栈顶，即this引用
            code_aload(0, out);

            // 获取父类的InvocationHandler引用，压入栈顶
            ...
            if (parameterTypes.length > 0) {
                ...
                for (int i = 0; i < parameterTypes.length; i++) {
                    ...
                    //如果是基本数据类型，则用包装类包装后入栈；如果是引用数据类型，则直接入栈
            ...
            /**
             * 调用InvocationHandler接口的invoke方法，将栈顶元素赋值给入参中靠后的参数
             */
            ...
            if (returnType == void.class) {
                ...
                //将InvocationHandler接口的invoke方法的返回值转换成正确的类型后返回
                //如果返回值是基本数据类型，则需要通过包装类的方法获取对应的基本数据类型的数值
         ...
         * 生成用于包装给定类型的参数的代码，其值可以在指定的局部变量索引处找到，
         * 以便将其作为Object传递给调用处理程序的“invoke”方法
         */
                ...
                throws IOException
        {
            ...
            //在常量池中添加setAccessible方法
            cp.getClass("java/lang/reflect/Method");
            cp.getNameAndType("setAccessible", "(Z)V");
            cp.getFieldRef(dotToSlash(className),
                    methodFieldName,
                    "Ljava/lang/reflect/Method;");
            cp.getMethodRef("java/lang/reflect/Method",
                    "setAccessible",
                    "(Z)V");
    ...
    private MethodInfo generateConstructor() throws IOException {
        ...
        /**
         * 给类型为InvocationHandler的全局变量h赋值
         */
        code_aload(0, out);
        code_aload(1, out);
        out.writeByte(opc_putfield);
        out.writeShort(cp.getFieldRef(this.className, "h",
                "Ljava/lang/reflect/InvocationHandler;"));

        // 从当前方法返回void
        out.writeByte(opc_return);

        // 赋值给stack、locals，说明方法栈的大小以及本地变量的数量
        minfo.maxStack = 10;
        minfo.maxLocals = 2;
        // 赋值给异常表
...
//  }
     ...
     * exception in the returned list is guaranteed to not be a supClass of
     ...
     * supClasses will never appear in the returned list.
     *
        ...
        for (Class<?> ex: exceptions) {
            ...
            for (int j = 0; j < uniqueList.size();) {
                ...
                if (ex2.isAssignableFrom(ex)) {
                    ...
                    /*
                     * if a supClass of this exception is on the list
